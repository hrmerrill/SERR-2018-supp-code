...
m = spikeSlabGAM(formula = Y.norm ~ X1 + X2 + X3 + X4 + X5 +
                   X6 + X7 + X8 + X9 + X10 + X11 + X12 + x.s + z.s + X1:x.s:z.s,
                 data = BCO.dat,
                 mcmc = mcmc)

which.selected = function(x){
  marg.p = summary(x)$trmSummary[-1,1][-(25:36)]
...
mny.terms = predict(m, type="terms")[-(25:36)]
...
M = "20170330_results_BGLGAM_ST.csv"
does.M.exist = file.exists(M)
if (!does.M.exist) file.create(M)
