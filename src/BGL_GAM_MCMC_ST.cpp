...
List M_BGL_SS_AR1_MCMC(arma::mat C_full,
  ...
  int n = Y.n_rows;
  arma::mat ID = arma::mat(size(DistS)); ID.eye();
  
  //precision matrix for data
  // arma::mat CorMat = exp(-pow(Dist/rho, 2)) + 1e-9*ID;
  arma::mat CorMat = exp(-abs(DistS/rhoS))%exp(-abs(DistT*rhoT));
  ...
  for(int i = 0; i < n_samples; i++){
    ...
    arma::colvec mu_g = Sigma_g*XT_Resids;
    w.rows(find(partition_ == 0)) = (sqrt(sig2)*root_Sig_g.t())*(as<arma::colvec>(rnorm(C_g.n_cols))) + mu_g;
    ...
    // sample rho
    double lk_curr = lk_Chol_arma(Y, C_full*w, sig2*CorMat) + log(rhoS) + log(rhoT);// + R::dgamma(rho, 1, 1/2, 1);
    ...
    arma::mat CorMat_prop = exp(-abs(DistS/rhoS_prop))%exp(-abs(DistT*rhoT_prop));
    
    double lk_prop = lk_Chol_arma(Y, C_full*w, sig2*CorMat_prop) + log(rhoS_prop) + log(rhoT_prop);// + R::dgamma(rho_prop, 1, 1/2, 1);
