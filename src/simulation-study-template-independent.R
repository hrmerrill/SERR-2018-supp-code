...
## of GAM model selection under independence
##
...
# dev.new()
# par(mfrow=c(3,4), mar=c(3,3,1,1), mgp=c(2,1,0))
# for(i in 1:n.true){
#   plot(sort(BCO.dat[,i+1]), trends[order(BCO.dat[,i+1]),i],
#        ylim=range(trends),
#        type = "l")
# }
...
fit.gam.dp = gam(Y.norm ~ ti(X1, k=num.knots.sim) + ti(X2, k=num.knots.sim) + ti(X3, k=num.knots.sim) + ti(X4, k=num.knots.sim) +
...
sel.model.Gaus.dp = ifelse(summary(fit.gam.dp)$s.pv < 0.05, 1, 0)
FP.Gaus.dp = sum(true.model.ind - sel.model.Gaus.dp == -1)
FN.Gaus.dp = sum(true.model.ind - sel.model.Gaus.dp == 1)

est.terms = predict(fit.gam.dp, type = "terms")
est.gaus.mean.dp = est.terms%*%sel.model.Gaus.dp + attr(est.terms, "constant")
MSE.gaus.dp = sqrt(mean((est.gaus.mean.dp - mean.trend)^2))

fit.gam.sh = gam(Y.norm ~ ti(X1, k=num.knots.sim, bs="cs") + ti(X2, k=num.knots.sim, bs="cs") + ti(X3, k=num.knots.sim, bs="cs") + ti(X4, k=num.knots.sim, bs="cs") +
...
sel.model.Gaus.sh = ifelse(summary(fit.gam.sh)$s.pv < 0.05, 1, 0)
FP.Gaus.sh = sum(true.model.ind - sel.model.Gaus.sh == -1)
FN.Gaus.sh = sum(true.model.ind - sel.model.Gaus.sh == 1)

est.terms.sh = predict(fit.gam.sh, type = "terms")
...
M = "20170330_results_BGLGAM_IND.csv"
does.M.exist = file.exists(M)
if (!does.M.exist) file.create(M)
